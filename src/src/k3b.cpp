...
void K3b::MainWindow::initView()
{
    ...
    //connect( viewImage, SIGNAL(clicked(bool)), this, SLOT(slotWriteImage()) );
    connect( viewCopy, SIGNAL(clicked(bool)), this, SLOT(slotMediaCopy()) );

    // --- Document Dock ----------------------------------------------------------------------------
    d->documentStack = new QStackedWidget( d->mainSplitter );//buttom 
    //d->documentStack = new QStackedWidget( );//buttom 
